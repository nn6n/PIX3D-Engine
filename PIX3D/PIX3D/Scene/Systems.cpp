...
namespace PIX3D
{
    ...
    void StaticMeshRendererSystem::Render(entt::registry& registry, GL::IBLMaps& ibl_maps, int point_lights_count)
    {
        ...
        view.each([&](TransformComponent& transform, StaticMeshComponent& mesh)
            {
                PIX3D::GL::GLRenderer::RenderMesh(transform.GetTransformMatrix(), mesh.m_Mesh, ibl_maps, point_lights_count);
            });
    }
